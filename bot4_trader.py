...
class Bot4Trader(BaseAgent):
    ...
    def get_analyst_rules(self):
        """Reads the dynamic rules generated by Bot 5."""
        rules_path = "/root/bot/dynamic_rules.json"
        if not os.path.exists(rules_path): rules_path = "dynamic_rules.json" # Local fallback
        
        try:
            if os.path.exists(rules_path):
                with open(rules_path, "r") as f:
                    return json.load(f)
            return {"max_spread_pips": 1.5, "risk_per_trade_percent": 1.0, "emergency_stop": False}
        except:
            return {"max_spread_pips": 1.5, "risk_per_trade_percent": 1.0, "emergency_stop": False}
    ...
    def execute_trade(self, pair: str, direction: str, price: float, sl: float, tp: float, reasoning: str, risk_percent: float = 1.0):
        self.logger.info(f"EXECUTING {direction} on {pair} @ {price} | Risk: {risk_percent}%")

        # In a real setup, units would be calculated based on SL distance and balance
        # For this skeleton, we use a fixed baseline scaled by Bot 5's risk percentage.
        units    = int(1000 * (risk_percent / 1.0)) if direction == "BUY" else int(-1000 * (risk_percent / 1.0))
        ...
        while True:
            ...
            try:
                ...
                if self.check_news_filter():
                    ...
                    # --- BOT 5 ANALYST CHECK ---
                    rules = self.get_analyst_rules()
                    curr_spread = float(self.get_current_spread("EUR_USD"))
                    
                    max_spread = float(rules.get("max_spread_pips", 1.5))
                    risk_val = float(rules.get("risk_per_trade_percent", 1.0))
                    
                    if rules.get("emergency_stop") is True:
                        self.logger.warning("EMERGENCY STOP ENABLED BY BOT 5. TRADING HALTED.")
                    elif curr_spread > max_spread:
                        self.logger.warning(f"SPREAD TOO HIGH: {curr_spread} vs Limit {max_spread}. Skipping.")
                    elif confirmation.get("action") == "execute" and confirmation.get("confidence", 0) > 0.75:
                        self.execute_trade(
                            pair=      "EUR_USD",
                            direction= "BUY",
                            price=     1.0850,
