...
class ProfessionalBacktesterV8:
    ...
    def run_backtest(self, ticker):
        ...
        for i in range(50, len(df_5m)):
            ...
            w = self.weights
            score = 0
            if row['FVG_Bull']: score += w.get('fvg', 20)
            if row['TurtleSoup_Bull']: score += w.get('turtle_soup', 25)
    ...
    def calculate_metrics(self):
        ...
        return {
            "win_rate": win_rate, "net_pnl": net_pnl, "max_dd": max_dd, 
            "sharpe": sharpe, "total_trades": len(df)
        }

if __name__ == "__main__":
    bt = ProfessionalBacktesterV8(max_trades=500)
    for t in ["EUR_USD", "GBP_USD", "XAU_USD", "NAS100_USD", "US30_USD"]:
        bt.run_backtest(t)
    ...
    exps.append({"timestamp": datetime.now().isoformat(), "performance": metrics})
